...
int array[MAX+MAX][MAX+MAX] ;	// Direction a[i][j] means that i indicates j 
				// if i,j<10 -> node to resource, else (i,j >= 10) -> resource to node 
unsigned long array_tid[MAX] ;	// Thread ID array
int address[MAX] ;		// mutex(resource) address 
...
main (int argc, char * argv[])
	...
	if(strcmp(in_lock,"lock") == 0)	
	{
	    ...
	    if(in_owner == 0) {
		// If someeone doens't lock, it will be allocated
		array[i][j] = 0 ;
		array[j][i] = 1 ;
	    }
